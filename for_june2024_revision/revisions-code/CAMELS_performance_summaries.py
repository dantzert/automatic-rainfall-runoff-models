...
os.chdir("C:/automatic-rainfall-runoff-models/for_june2024_revision")
print(os.getcwd())
...
with open("revisions-code/all_metrics.p", "rb") as fp:
...
folder_path = str('revisions-code/camels_modpods_results')
modpods_basins = set()
...
# print the entries in data with a float precision of 2
df = pd.DataFrame(data)
...
camels_error_metrics = pd.read_csv("revisions-code/camels_modpods_results/ensemble.csv",index_col=0)
# cast the index to strings to match "basins"
camels_error_metrics.index = camels_error_metrics.index.astype(str)
# drop any rows in camels_error_metrics where the index is not in "basins"
camels_error_metrics = camels_error_metrics[camels_error_metrics.index.isin(basins)]
camels_error_metrics = camels_error_metrics.drop(columns="model")

print("modpods CAMELS Median Values")
print(camels_error_metrics.median())

#print(omit_std)

SI_Table = omit_std
# also drop columns: "NSE mean", "failures"
SI_Table = SI_Table.drop(columns=["NSE mean", "failures"])
print("\nSI Table\n")
print(SI_Table)
