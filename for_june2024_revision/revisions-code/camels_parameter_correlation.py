...
attributes =  pd.read_csv(str("CAMELS/camels_" + attribute_names[0] + ".txt"),sep=';',index_col='gauge_id')
for idx in range(1,len(attribute_names)):
    ...
    temp = pd.read_csv(str("CAMELS/camels_" + attribute_names[idx] + ".txt"),sep=';',index_col='gauge_id')
...
p1t1 = pd.DataFrame(np.empty((0, 9  ) ) ) 
p1t2 = pd.DataFrame(np.empty((0, 9  ) ) )
p2t1 = pd.DataFrame(np.empty((0, 9  ) ) )
p2t2 = pd.DataFrame(np.empty((0, 9  ) ) )
p3t1 = pd.DataFrame(np.empty((0, 9  ) ) )
p3t2 = pd.DataFrame(np.empty((0, 9  ) ) )
...
indices_to_keep = [9,10,15,16,36,37,43,20,54,17,18,21]
p1t1_for_correlation = p1t1.iloc[:,indices_to_keep]

p1t1corr = p1t1_for_correlation.corr()


# print all columns of dataframes
pd.set_option('display.max_columns', None)

print("pearson's r for p1t1")
print(p1t1corr)
